...
So, X $\in$ [0,$\infty$) \\
Using the probability in exponential distribution,
\begin{align}
\label{CDF}
\text{CDF of X is F(y)}  = \pr{X \le y} \\
\label{Int}
\implies F(y) = 1 -  \int_{y}^{\infty} \frac{1}{5}e^{-\frac{x}{5}} \,dx \\
\label{P}
\text{Required Probability is } \pr{X > 5} = 1 - \pr{X \le 5} 
\end{align}
From \eqref{CDF}, \eqref{Int} and \eqref{P}
\begin{align}
\implies \pr{X > 5} = 1 - \brak{1 -  \int_{5}^{\infty} \frac{1}{5}e^{-\frac{x}{5}} \,dx}\\
\implies \pr{X > 5} = \int_{5}^{\infty}\frac{1}{5}e^{-\frac{x}{5}} \\
\implies \pr{X > 5} = \brak{-e^{- \frac{x}{5}}} \Bigg|_5^\infty \\
\implies \pr{X > 5} = -e^{-\infty} + e^{-1} \\
\therefore \pr{X > 5} = 0 + e^{-1} = \frac{1}{e} 
\end{align}
